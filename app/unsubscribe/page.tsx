'use client';
import { useState, useEffect, Suspense } from 'react';
import { useSearchParams } from 'next/navigation';
import { INK, INK_SOFT, INK_FAINT, LINE, PAPER, CARD, ACCENT, CLAY, SERIF, SANS } from '../lib/theme';

function Nav() {
  return (
    <nav style={{ background: CARD, padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${LINE}`, position: 'sticky', top: 0, zIndex: 100 }}>
      <a href="/" style={{ textDecoration: 'none', fontFamily: SERIF, fontSize: 18, fontWeight: 500, color: INK }}>jobsesame</a>
    </nav>
  );
}

function Success() {
  return (
    <>
      <div style={{ fontSize: 48, marginBottom: 20 }}>✓</div>
      <h1 style={{ fontFamily: SERIF, fontSize: 24, fontWeight: 500, color: INK, marginBottom: 12, marginTop: 0 }}>You&apos;ve been unsubscribed</h1>
      <p style={{ fontSize: 14, color: INK_SOFT, lineHeight: 1.7, marginBottom: 28 }}>
        You have been unsubscribed from Jobsesame marketing emails. You may still receive transactional emails about your account and applications.
      </p>
      <a href="/" style={{ fontSize: 13, color: ACCENT, textDecoration: 'none' }}>Back to Jobsesame →</a>
    </>
  );
}

function UnsubscribeContent() {
  const searchParams = useSearchParams();
  const token = searchParams.get('token');

  const [status, setStatus] = useState<'idle' | 'loading' | 'done' | 'error'>('idle');
  const [errorMsg, setErrorMsg] = useState('');

  // Token path: auto-unsubscribe on mount
  useEffect(() => {
    if (!token) return;
    setStatus('loading');
    fetch(`/api/user/unsubscribe?token=${encodeURIComponent(token)}`, { method: 'POST' })
      .then(async res => {
        if (res.ok) {
          setStatus('done');
        } else {
          const d = await res.json().catch(() => ({}));
          setErrorMsg(d.error || 'Something went wrong. Please try again.');
          setStatus('error');
        }
      })
      .catch(() => {
        setErrorMsg('Something went wrong. Please try again.');
        setStatus('error');
      });
  }, [token]);

  // --- Token path UI ---
  if (token) {
    if (status === 'loading' || status === 'idle') {
      return (
        <>
          <div style={{ fontSize: 36, marginBottom: 20 }}>⏳</div>
          <h1 style={{ fontFamily: SERIF, fontSize: 22, fontWeight: 500, color: INK, marginBottom: 12, marginTop: 0 }}>Unsubscribing…</h1>
          <p style={{ fontSize: 14, color: INK_SOFT }}>Please wait a moment.</p>
        </>
      );
    }
    if (status === 'done') return <Success />;
    return (
      <>
        <div style={{ fontSize: 36, marginBottom: 20 }}>⚠️</div>
        <h1 style={{ fontFamily: SERIF, fontSize: 22, fontWeight: 500, color: INK, marginBottom: 12, marginTop: 0 }}>Link expired</h1>
        <p style={{ fontSize: 14, color: CLAY, lineHeight: 1.7, marginBottom: 28 }}>{errorMsg}</p>
        <p style={{ fontSize: 13, color: INK_SOFT }}>
          Sign in to your account to manage email preferences, or{' '}
          <a href="mailto:hello@jobsesame.co" style={{ color: ACCENT }}>contact support</a>.
        </p>
      </>
    );
  }

  // --- No-token path: manual form ---
  return <ManualForm />;
}

function ManualForm() {
  const [email, setEmail] = useState('');
  const [done, setDone] = useState(false);
  const [loading, setLoading] = useState(false);
  const [authError, setAuthError] = useState(false);
  const [error, setError] = useState('');

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!email) return;
    setLoading(true);
    setError('');
    setAuthError(false);
    try {
      const res = await fetch('/api/user/unsubscribe', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email }),
      });
      if (res.status === 401) { setAuthError(true); setLoading(false); return; }
      if (!res.ok) { const d = await res.json(); setError(d.error || 'Something went wrong'); setLoading(false); return; }
      setDone(true);
    } catch {
      setError('Something went wrong. Please try again.');
    }
    setLoading(false);
  }

  if (done) return <Success />;

  return (
    <>
      <h1 style={{ fontFamily: SERIF, fontSize: 26, fontWeight: 500, color: INK, marginBottom: 12, marginTop: 0 }}>Unsubscribe from emails</h1>
      <p style={{ fontSize: 14, color: INK_SOFT, lineHeight: 1.7, marginBottom: 32 }}>
        Enter your email address below to unsubscribe from Jobsesame marketing emails. Note: you will still receive important account and security emails.
      </p>
      <form onSubmit={handleSubmit}>
        <div style={{ background: CARD, border: `1px solid ${LINE}`, borderRadius: 8, padding: '28px', textAlign: 'left' }}>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: INK_SOFT, marginBottom: 8, letterSpacing: '0.5px', textTransform: 'uppercase' }}>
            Email address
          </label>
          <input
            type="email"
            required
            value={email}
            onChange={e => setEmail(e.target.value)}
            placeholder="you@example.com"
            style={{
              width: '100%', background: PAPER, border: `1.5px solid ${LINE}`,
              borderRadius: 6, padding: '13px 16px', fontSize: 14, color: INK,
              outline: 'none', boxSizing: 'border-box', marginBottom: 20,
            }}
          />
          {authError && (
            <div style={{ background: 'rgba(168,92,64,0.08)', border: `1px solid rgba(168,92,64,0.3)`, borderRadius: 6, padding: '12px 16px', marginBottom: 16, fontSize: 13, color: CLAY, lineHeight: 1.6 }}>
              You need to be signed in to unsubscribe.{' '}
              <a href="/sign-in" style={{ color: ACCENT, fontWeight: 700 }}>Sign in →</a>
            </div>
          )}
          {error && (
            <div style={{ background: 'rgba(168,92,64,0.08)', border: `1px solid rgba(168,92,64,0.3)`, borderRadius: 6, padding: '12px 16px', marginBottom: 16, fontSize: 13, color: CLAY }}>
              {error}
            </div>
          )}
          <button
            type="submit"
            disabled={!email || loading}
            style={{
              width: '100%', background: ACCENT, color: PAPER,
              fontWeight: 700, fontSize: 15, border: 'none', borderRadius: 3,
              padding: '15px 24px', cursor: email && !loading ? 'pointer' : 'not-allowed',
              opacity: email && !loading ? 1 : 0.5,
            }}
          >
            {loading ? 'Unsubscribing...' : 'Unsubscribe'}
          </button>
        </div>
      </form>
      <p style={{ fontSize: 12, color: INK_FAINT, marginTop: 20, lineHeight: 1.7 }}>
        <a href="/privacy" style={{ color: ACCENT }}>Privacy Policy</a>
        {' · '}
        <a href="/delete-data" style={{ color: ACCENT }}>Delete all my data</a>
      </p>
    </>
  );
}

export default function UnsubscribePage() {
  return (
    <main style={{ fontFamily: SANS, background: PAPER, color: INK, minHeight: '100vh' }}>
      <Nav />
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '72px 24px 80px', textAlign: 'center' }}>
        <Suspense fallback={
          <p style={{ color: INK_SOFT, fontSize: 14 }}>Loading…</p>
        }>
          <UnsubscribeContent />
        </Suspense>
      </div>
    </main>
  );
}
